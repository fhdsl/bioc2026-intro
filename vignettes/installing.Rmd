...
With over 3500 packages, it can be difficult to find a package that does what you need. To help with this, Bioconductor has the [biocViews page](https://bioconductor.org/packages/release/BiocViews.html#___Software) to partition packages first by type (Software/AnnotationData/ExperimentData/Workflow) and then the packages are listed below in order of how often they are downloaded. Clicking on the arrow next to any type shows further subdivision and clicking on any one will restrict the package list below. In the "Find biocViews:" search box you can start typing in what you want like "rna" and a popup box showing the defined biocViews terms that match it. In the packages section you can also use the "Search table:" to find packages that match in the package name, Maintainer or Title columns. Try putting in **DESeq2**.
...
A **vignette** shows how to use the package to run a typical analysis from start to finish. This is much more useful than the reference manual, often with a runnable analysis with internal data. This allows a user to run through working code, and later modify it to fit their own data. Many package authors use their vignettes to run package demos at various conferences, and videos of many of these are available on [Bioconductor's YouTube channel](https://www.youtube.com/user/bioconductor).

## Workflow packages

One of the 4 main types of packages on the biocViews, the [Workflow packages](https://bioconductor.org/packages/release/BiocViews.html#___Workflow) maybe the most useful when trying to learn how to analyze a particular type of data. Most end-to-end analyses will use many different packages, and the workflow packages walk through all the typical steps and important asides. They are good for self-study as they also contain runnable code with example data, like vignettes, that you can modify for your own data. 
