...
- [A
  ...
  of curated, inter-operable R packages
...
For manipulating the `SummarizedExperiment` object, we will use the
`tidySummarizedExperiment` package to subset it - essentially this
...
You can see we hve 19 rows left in `subset_SE` that map to our Genomic
Range of interest.
...
of the `SummarizedExperiment`. They might add some functionality that is
